...
public class RollingFileAppender extends FileAppender {
    ...
    void rollOver() {
        ...
        if(maxBackupIndex > 0) {
            ...
            file = new File(fileName.replaceFirst(".out", "") + maxBackupIndex + ".out");
            if (file.exists())
                renameSucceeded = file.delete();

            // Map {(maxBackupIndex - 1), ..., 2, 1} to {maxBackupIndex, ..., 3, 2}
            for (int i = maxBackupIndex - 1; i >= 1 && renameSucceeded; i--) {
                file = new File(fileName.replaceFirst(".out", "") + i + ".out");
                if (file.exists()) {
                    target = new File(fileName.replaceFirst(".out", "") + (i + 1) + ".out");
